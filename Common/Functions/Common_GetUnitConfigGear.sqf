...
if (_backpack != "") then {
	if (isClass(configFile >> 'CfgVehicles' >> _backpack >> 'TransportWeapons')) then {
		...
		for '_j' from 0 to count(configFile >> 'CfgVehicles' >> _backpack >> "TransportWeapons")-1 do {
			...
			if !(isNil '_get') then {
				[(_get_backpack_content select 0) select 0, _cname] Call WFBE_CO_FNC_ArrayPush;
				[(_get_backpack_content select 0) select 1, getNumber(_root_sub >> 'count')] Call WFBE_CO_FNC_ArrayPush;
			};
		};
	};
	if (isClass(configFile >> 'CfgVehicles' >> _backpack >> 'TransportMagazines')) then {
		...
		for '_j' from 0 to count(configFile >> 'CfgVehicles' >> _backpack >> "TransportMagazines")-1 do {
			...
			if !(isNil '_get') then {
				[(_get_backpack_content select 1) select 0, _cname] Call WFBE_CO_FNC_ArrayPush;
				[(_get_backpack_content select 1) select 1, getNumber(_root >> 'count')] Call WFBE_CO_FNC_ArrayPush;
			};
		};
	};
};
