...
//--- Create.
{
	...
	if (_canCreate) then {
		if (_x isKindOf 'Man') then {
			...
			if (_vehicle emptyPositions 'driver' > 0) then {_driver = [_type,_team,_position,_sideID] Call WFBE_CO_FNC_CreateUnit;_driver moveInDriver _vehicle;_driver addeventhandler ["HandleDamage",format ["_this Call %1", _rearmor]];_crews = _crews + [_driver]};
			if (_vehicle emptyPositions 'gunner' > 0) then {_gunner = [_type,_team,_position,_sideID] Call WFBE_CO_FNC_CreateUnit;_gunner moveInGunner _vehicle;_gunner addeventhandler ["HandleDamage",format ["_this Call %1", _rearmor]];_crews = _crews + [_gunner]};
			if (_vehicle emptyPositions 'commander' > 0) then {_commander = [_type,_team,_position,_sideID] Call WFBE_CO_FNC_CreateUnit;_commander addeventhandler ["HandleDamage",format ["_this Call %1", _rearmor]];_commander moveInCommander _vehicle;_crews = _crews + [_commander]};
